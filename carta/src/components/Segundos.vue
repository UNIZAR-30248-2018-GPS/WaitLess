...
<script>
  ...
  export default {
    ...
    data(){
      return{
        items: this.$session.get('segundos'),
      }
    },
    ...
    methods: {
      /**
       * Función que pide los productos tipo menu2 (segundos platos) a la api
      ...
      fetchItems() {
        ...
        axios({
          method: 'get',
          url: 'http://localhost:3030/api/carta?tipo=menu2'}
        ).then(response => {
            this.$session.set('segundos',response.data);
            this.items = this.$session.get('segundos');
